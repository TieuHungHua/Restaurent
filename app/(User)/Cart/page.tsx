...
const Cart = ({ total, onApplyCoupon, onCheckout }: CartSummaryProps) => {
  ...
  const handleCheckout = async () => {
    const cartItem: { dishId: string; number: number }[] = itemCart.map(
      (item) => {
        return { dishId: item.id, number: Number(item.quantity) };
      }
    );
    ...
    const fetchDataD = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8000/api/v1/cart/remove-dish/${id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div className="flex flex-row justify-center mt-5 gap-1">
      ...
      <div>
        <PaymentSumary {...userInfor} setUserInfor={setUserInfor} />
      </div>
    </div>
  );
};
  ...
  return (
    <div className="p-4 mt-4 border rounded-md shadow-sm">
      <div className="flex justify-between items-center mb-3">
        <h3 className="font-normal text-[1rem] text-gray-700">
          Tổng tiền hàng
        </h3>
        <span className="font-normal text-[1rem] text-gray-700">
          {total.toLocaleString()},000 VND
        </span>
      </div>
      <div className="flex justify-between items-center mb-3">
        <h3 className="font-normal text-[1rem] text-gray-700">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <span className="font-normal text-[1rem] text-red-600">
          -đ {10},000 VND
        </span>
      </div>
      <div className="flex justify-between items-center mb-3">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <span className="font-bold text-xl">
          {(total + 10 - 10 - 10).toLocaleString()},000 VND
        </span>
      </div>
