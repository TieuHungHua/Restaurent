...
const MenuPage: React.FC = () => {
  ...
  const searchFood = async () => {
    try {
      const res = await fetch(
        `http://localhost:8000/api/v1/dish/search-by-name?name=${foodSearch}`,
        {
          method: "GET",
          headers: {
  ...
  const fetchDataFood = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/v1/dish/guest?page=${from}&limit=8`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAddToCart = async (id: string) => {
    try {
      const response = await fetch(
        "http://localhost:8000/api/v1/cart/add-dish",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="flex m-h-[150vh">
      ...
      <main style={{ flex: 1, padding: "20px", marginBottom: "10px" }}>
        <div className="flex flex-row relative w-full max-w-3xl mb-5 ">
          ...
          <Button
            onClick={searchFood}
            className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-4 py-2 ml-3 rounded-md"
          >
            Tìm kiếm
          </Button>
        </div>
        <div>
          ...
          {isLoading ? (
            <div className="flex justify-center w-[750px] mt-6 space-x-2">
              ...
              {/* Trang sau */}
              <button
                onClick={() => setFrom((prev) => Math.min(prev + 1, totalPage))}
