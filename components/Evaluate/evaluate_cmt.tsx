...
const ReviewItem: React.FC<ReviewItemProps> = ({ comment }) => {
    return (

        <div className="p-4 bg-gray-50 rounded-md border space-y-2">
            <div className="flex items-center justify-between">
                <span className="font-semibold">Khách hàng {comment.userName}</span>
            ...
            <div className="text-xs text-gray-500">
                Đăng vào lúc: {format(comment.createdAt, "yyyy-MM-dd HH:mm:ss")}
            </div>

            <div className="flex items-center space-x-4 text-sm">
                <div className="flex items-center space-x-1 text-yellow-600">
                    <ThumbsUp className="w-4 h-4" /> <span>{comment.likes}</span>
                </div>
                <div className="flex items-center space-x-1 text-red-600">
                    <ThumbsDown className="w-4 h-4" /> <span>{comment.dislikes}</span>
                </div>
            </div>

            <Button
                variant="ghost"
                className="p-0 text-red-500 hover:underline text-sm"
                onClick={onDelete}
            >
                Xoá đánh giá
            </Button>
